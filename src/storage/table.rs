...
pub struct Table {
    ...
    pub row_count: u64,
    pager: Pager,
    /// B-tree index on the primary key (id) → row slot number.
    ///
    /// The slot number is the logical row index starting from 0.
    index: Btree<usize>,
}
...
impl Table {
    pub fn open(path: &str, schema: Schema) -> Result<Self, std::io::Error> {
        ...
        // Start with an empty in-memory index; we will populate it for
        // existing tables below.
        let index = Btree::new(2);
        ...
        // Existing table on disk — rebuild the in-memory index by
        // scanning all stored rows once.
        table.rebuild_index()?;
    ...
    pub fn insert(&mut self, row: &Row) -> Result<(), std::io::Error> {
        ...
        // Update in-memory index so future searches and scans see this row.
        self.index.insert(row.id, slot);
    ...
    pub fn scan(&mut self) -> Result<Vec<Row>, std::io::Error> {
        ...
        // Iterate over the index so rows come back sorted by primary key (id),
        // regardless of insertion order.
        for key in self.index.keys_in_order() {
            if let Some(slot_ref) = self.index.search(key) {
                let slot = *slot_ref;
                let page_no = 1 + (slot / ROWS_PER_PAGE) as u64;
                let slot_in_page = slot % ROWS_PER_PAGE;
    ...
    /// Rebuild the in-memory B-tree index from the rows stored on disk.
    fn rebuild_index(&mut self) -> Result<(), std::io::Error> {
        self.index = Btree::new(2);
