// Page layout:
//   Page 0  — metadata: [row_count: 8][schema: SCHEMA_SIZE][padding...]
//   Pages 1+ — row data: rows packed tightly, ROWS_PER_PAGE rows per page.
//
// A row slot is addressed by (page_no, slot_index) where page_no >= 1.

use super::pager::{Pager, PAGE_SIZE};
use crate::row::{Row, Schema, ROW_SIZE, SCHEMA_SIZE};
