...
use crate::row::{Row, Schema, MAX_COLS};
...
impl Engine {
    ...
    fn exec_create(
        ...
        // id is implicit; user columns map to the MAX_COLS slots
        let user_cols: Vec<&str> = columns.iter().map(|c| c.name.as_str()).collect();
        if user_cols.len() > MAX_COLS {
    ...
    fn exec_insert(
        ...
        let expected = 1 + table.schema.col_count;
